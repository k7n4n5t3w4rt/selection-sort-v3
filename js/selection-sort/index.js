...
export function selectionSortFactory (
  ...
  function skipToNextLoop (
    ...
    setTimeout(() => {
      D.clearActiveCellsDisplay(i, minIndex, config.CONTAINER_ID, config.SHOW_WORKING)
      loop(a, ++i)
  ...
  function swapAndLoopAgain (
    ...
    setTimeout(() => {
      D.swapCells(
        ...
        .then(() => {
          D.swapActiveCellsDisplay(i, minIndex, config.CONTAINER_ID, config.SHOW_WORKING)
  ...
  function findMinIndex (
    ...
    return new Promise((resolve) => {
      const intervalID = setInterval(() => {
        D.setCellDisplay(j, 'remove', 'actively-looking', config.CONTAINER_ID, config.SHOW_WORKING)
        ++j
        if (j >= a.length) {
          clearInterval(intervalID)
          D.setCellDisplay(minIndex, 'remove', 'min', config.CONTAINER_ID, config.SHOW_WORKING)
          D.setCellDisplay(minIndex, 'remove', 'actively-looking', config.CONTAINER_ID, config.SHOW_WORKING)
          D.setCellDisplay(minIndex, 'add', 'active-min', config.CONTAINER_ID, config.SHOW_WORKING)
          return resolve(minIndex)
        }
        D.setCellDisplay(j, 'add', 'actively-looking', config.CONTAINER_ID, config.SHOW_WORKING)
        if (a[j].value < minValue) {
          D.setCellDisplay(minIndex, 'remove', 'min', config.CONTAINER_ID, config.SHOW_WORKING)
          minValue = a[j].value
          minIndex = j
          D.setCellDisplay(minIndex, 'add', 'min', config.CONTAINER_ID, config.SHOW_WORKING)
        }
      }, config.CLICK * 1) // eslint-disable-line no-undef
    })
  }
